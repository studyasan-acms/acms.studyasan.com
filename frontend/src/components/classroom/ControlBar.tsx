...
export function ControlBar({
    ...
    return (
        <div className="relative flex flex-col items-center">
            {/* Horizontal Floating Emoji Reactions Bar */}
            {showEmojiPicker && (
            ...
            <div className="flex items-center gap-1 md:gap-2 p-2 rounded-2xl md:rounded-full bg-white/95 backdrop-blur-xl border border-slate-200 shadow-xl max-w-[95vw] overflow-x-auto scrollbar-hide no-scrollbar">
                ...
                {/* Volume / Sound Toggle (Direct on/off) */}
                <Button
                    variant={isAudioClosed ? 'destructive' : 'outline'}
                    size="icon"
                    className={`h-10 w-10 rounded-full transition-all ${!isAudioClosed
                        ? 'hover:bg-slate-100 text-slate-700 border-slate-200'
                        : 'bg-red-500 hover:bg-red-600 border-red-500 text-white'
                        }`}
                    onClick={onToggleAudioClosed}
                    disabled={!isConnected}
                    title={isAudioClosed ? 'Turn On Volume (Unmute Sound)' : 'Turn Off Volume (Mute Sound)'}
                >
                    {isAudioClosed ? <VolumeX className="h-4 w-4" /> : <Volume2 className="h-4 w-4" />}
                </Button>
