...
import { Plus, Trash2, Edit, Search, GraduationCap, Loader2, ArrowUpDown, ArrowUp, ArrowDown } from 'lucide-react';
...
const ClassesPage: React.FC = () => {
  ...
  const canDeleteClass = isAdmin || canDeletePerm('classes');
  ...
  const handleFormSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (selectedClass) {
        await classService.update(selectedClass.id, { name: formData.name });
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      ...
      setErrorMessage('Failed to delete class. It may be in use.');
      setShowErrorModal(true);
    }
  };
      ...
      <Dialog open={showFormModal} onOpenChange={setShowFormModal}>
        <DialogContent className="sm:max-w-[425px]">
          <DialogHeader>
            ...
            <DialogDescription>
              {selectedClass ? 'Update the details of the class.' : 'Add a new class to the system.'}
            </DialogDescription>
          </DialogHeader>
          <form onSubmit={handleFormSubmit} className="space-y-4 py-4">
            <div className="space-y-2">
              <Label htmlFor="class-name">Class Name <span className="text-red-500">*</span></Label>
              <Input
                id="class-name"
